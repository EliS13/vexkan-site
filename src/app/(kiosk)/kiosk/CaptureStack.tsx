...
export function CaptureStack({ photos, of }: { photos: string[]; of: number }) {
  ...
  return (
    <div className="absolute bottom-3 left-3 flex items-center gap-2 rounded-full bg-k-paper/80 py-1.5 pr-3 pl-2">
      <div className="flex">
        {photos.map((photo, i) => (
          ...
          <img
            ...
            style={{ zIndex: i }}
            className={`relative size-9 rounded-md border-2 border-k-grass object-cover ${
              i === 0 ? "" : "-ml-4"
            }`}
          />
        ))}
      </div>
      <span className="font-mono text-[11px] tabular-nums text-k-sketch">
        {photos.length} of {of}
      </span>
    </div>
  );
